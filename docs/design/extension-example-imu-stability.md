...
| **Filter handles it** | Modern EKFs estimate bias online, so they don't need Allan specs as inputs |
...
- **Units:** Always explicit via `unit` attribute (no defaults, because these parameters
